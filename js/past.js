    ...
    fetch(URL)
        .then (respuesta => respuesta.json())
            .then(data =>{
                ...
                creacionDeCheckboxes(filtrarAlfabeticamente(filtroDeFecha(eventos, fecha)))
                impresionDeTarjetas(filtrarAlfabeticamente(filtroDeFecha(eventos, fecha)))
                dataDeEventos = filtrarAlfabeticamente(filtroDeFecha(eventos, fecha))
            })
    }
...
function filtroDeFecha(eventos, fecha){ 
  let filtrados = eventos.filter((evento)=> evento.date < fecha)
  return filtrados
}
